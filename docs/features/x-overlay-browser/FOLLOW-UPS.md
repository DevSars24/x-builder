...
| F4 | enhancement | carried D1 (rule-of-three) | ~~**v2 `Button` `block`/`fullWidth` + `borderColor`/edge-token prop**~~ | ✅ **DONE** — `overlay/src/ui/v2/button.tsx` now has a `block` prop + variant border tokens (`--xb-border-edge`); wrapper-span workarounds can be retired by consumers as they're touched. |
| F5 | feature | re-validation | ~~**Suggest home/profile route-gate.**~~ | **MOOT for now** — the `SuggestController` was subsequently **unmounted** in `runtime.tsx` (it read as a confusing second always-on orb beside Settings). The feature code is parked for folding into the Settings panel or compose flow; the route-gate only matters if/when it's re-mounted. |
